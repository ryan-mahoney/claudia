...
module.exports = function collectFiles(
  ...
  const logger = optionalLogger || new NullLogger(),
    ...
    npmOptions =
      options && options["npm-options"]
        ? options["npm-options"].split(" ")
        : [],
    ...
    cleanCopyToDir = function(projectDir) {
      return packProjectToTar(projectDir, workingDir, npmOptions, logger)
        ...
        .then(dir =>
          copyIfExists(dir, projectDir, [".npmrc", "package-lock.json"])
        );
    },
    ...
    hasRelativeDependencies = function(packageConf) {
      return ["dependencies", "devDependencies", "optionalDependencies"].find(
        depType => {
    ...
    remapSingleDep = function(dependencyPath, referencePath) {
      ...
      if (fsUtil.isDir(actualPath)) {
        if (!activeRemapPromise[actualPath]) {
          activeRemapPromise[actualPath] = readjson(
            path.join(actualPath, "package.json")
          )
            .then(packageConf => {
              if (!hasRelativeDependencies(packageConf)) {
                return packProjectToTar(
                  actualPath,
                  workingDir,
                  npmOptions,
                  logger
              ...
              return cleanCopyToDir(actualPath)
                .then(cleanCopyPath =>
                  rewireRelativeDependencies(cleanCopyPath, actualPath)
      ...
      throw new Error(
        `${dependencyPath} points to ${actualPath}, which is neither dir nor a file, cannot remap.`
      );
    },
    remapDependencyType = function(subConfig, referenceDir) {
      ...
      return Promise.all(
        relativeDeps.map(key => remapSingleDep(subConfig[key], referenceDir))
      )
        .then(results =>
          results.forEach(
            (val, index) => (subConfig[relativeDeps[index]] = val)
          )
        )
        .then(() => true);
    },
      ...
      return readjson(confPath)
        .then(packageConfig => {
          if (hasRelativeDependencies(packageConfig)) {
            ...
            return Promise.all(
              ["dependencies", "optionalDependencies"].map(t =>
                remapDependencyType(packageConfig[t], referenceDir)
              )
            )
              .then(() =>
              ...
              .then(() =>
                fsUtil.silentRemove(path.join(targetDir, "package-lock.json"))
              );
          }
        })
        .then(() => targetDir);
